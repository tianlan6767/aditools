...
def query_image(connection: pymysql.Connection, batch_ids: Union[int, List[int]], save_type='json', save_path=None,
                save_image=False, save_thumb=True, image_folders: Optional[List[str]] = None, image_format=".jpg",
                QC_data: Optional[str] = None, retrain_filtered=False, split_thresh=3000):
    ...
    sql = f"""Select image.image_path, image.work_no, image.camera_no, image.image_no, image.create_time, 
        ...
        dft.f_mark_c, dft.f_mark_r, dft.soft_judge, dft.mask_point_c, dft.mask_point_r, dft.width, dft.length
    ...
    cursor.close()
    print(f'查询完成-{len(defect_infos)}条缺陷')
    ...
    def _parse_sql_info(sql_info):
        ...
        _batch_id, _product_No, _name = _path_infos[-5], _path_infos[-3], _path_infos[-1]
        _data_key = f'{_batch_id}_{_product_No}_{_name}.bmp'
    ...
    if qc_data and os.path.exists(qc_data):
        ...
        for k, v in defect_data.items():
            if k in check_data:
                    ...
                    for box in check_data[k]['regions']:
                        if box['shape_attributes']['name'] == 'polygon':
                            judge = box_contains_point(box['shape_attributes']['all_points_x'],
                                                       ...
                                                       (x, y))
                        elif box['shape_attributes']['name'] == 'rect':
                            judge = box_contains_point([box['shape_attributes']['x'],
                                                        box['shape_attributes']['x']+box['shape_attributes']['width']],
                                                       [box['shape_attributes']['y'],
                                                        box['shape_attributes']['y']+box['shape_attributes']['height']],
                                                       (x, y))
                        if judge == 1:
                            break
                ...
                miss_count += 1
        assert miss_count != len(defect_data), '检查QC复判数据，QC数据与当前查询数据不匹配'
    elif qc_data:
        print('缺陷复判数据不存在！')
...
def save_data(defect_data: dict, thumb_data: dict, save_type, save_path, save_name):
    ...
    if save_type in ['excel', 'all']:
        ...
        with tqdm(range(len(dft_data) + 1), desc=f'保存Excel[{save_name}]数据', ncols=80, delay=0.05) as dft_pro:
            for idx in dft_pro:
                if idx != len(dft_data):
...
def draw_defect(image_file: str, dft_data: dict, save_folder, compared=True, draw_label=False, draw_single=True,
                color=(255, 0, 0), thickness=1, save_format='.bmp', thumbnail=False):
    """
...
if __name__ == '__main__':
    ...
    with connect_database(host='192.168.2.29') as db_connect:
        # print(query_production(db_connect, 6))
        query_image(db_connect, 1479, save_type='all', save_image=True, save_thumb=True,
                    retrain_filtered=False,
                    split_thresh=5000,
                    image_folders=[
                        r"\\192.168.2.54\work\Y\836灰\836灰-政秀整理-重新编号\按格式放好"],
                    QC_data=r"\\192.168.2.54\work\Y\836灰\836灰-政秀整理-重新编号\data-1479.json",
                    save_path=r"\\192.168.2.54\work\Y\836灰\836灰-政秀整理-重新编号\dbutil_result"
                    )
