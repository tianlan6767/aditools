...
from dpp.common.util import flatten, save_json, cal_area, parse_region
...
# from dpp.common.log import stdlog
...
def del_small_area(json_data, min_area):
    ...
    for key in list(dpp.dataset.keys()):
...
def filter_labels(json_data, labels):
    ...
    dataset = deepcopy(json_data)
    for key, value in dpp.dataset.items():
