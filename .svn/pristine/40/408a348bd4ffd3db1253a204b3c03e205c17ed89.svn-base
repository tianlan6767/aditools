...
def rename_img(imgs, dst, start):
    for img in tqdm(imgs):
        ...
        fmt = fn.split('.')[-1]
        newname = '{}{}.{}'.format(start, fn.replace(".bmp", "").replace(".jpg", ""),fmt)
...
def correct_img_json(imgs,json_data,dst):
    for img in imgs:
        filename = os.path.basename(img)
        fmt = filename.split(".")[-1]
        res1 = re.findall('[\u4e00-\u9fa5]', filename)
        for item in res1:
            filename=filename.replace(item,"")
        res2 = re.findall('(.*?)[-_]\.[jb][pm][gp]', filename)
        if len(res2):
            filename = res2[0]+"."+fmt
        json_data = json_data.replace(os.path.basename(img),filename)
        shutil.copy(img,os.path.join(dst,filename))
    return json_data
